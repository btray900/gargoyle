...
def main(gargoyleFunctions):
    ...
    # GET list of components for options
    c = requests.get(url = 'http://db-api:5000/api/v1/components')
    comp = json.loads(c.text)
    componentStr = '%s' % comp['payload']
    components = componentStr.split(',')

    # help
    parser = argparse.ArgumentParser(description='Run OSG checks against host list')
    parser.add_argument('-c', '--component', nargs='?', choices=components, required=False, const='identity', help='Test by component (https://docs.openstack.org/security-guide/). Default: identity.',)
    ...
    for host in hosts:
        ...
        if singleComponent:
            ...
            securityChecks[host].append(commonHandler.getChecks('identity'))
            securityChecks[host].append(commonHandler.getChecks('dashboard'))
            securityChecks[host].append(commonHandler.getChecks('compute'))
            securityChecks[host].append(commonHandler.getChecks('blockStorage'))
            securityChecks[host].append(commonHandler.getChecks('objectStorage'))
            securityChecks[host].append(commonHandler.getChecks('networking'))
            securityChecks[host].append(commonHandler.getChecks('imageStorage'))
